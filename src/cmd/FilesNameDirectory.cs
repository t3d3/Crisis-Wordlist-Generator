...
namespace crisis
{
    public class FilesNameDirectory
    {
        ...
        public void FilesPathDirectory()
        {
            if (OsDetect() == true)
            {
                filePath [0] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"/output-dico/charset";
                filePath [1] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"/output-dico/wordlist";
                filePath [2] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"/output-dico/wordlist";
                filePath [3] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"/output-dico/Leet_Speak";
                fileName [0] = @"/charset_";
                fileName [1] = @"/wordlist_";
                fileName [2] = @"/wordlist_";
                fileName [3] = @"/dico1337_";
            } else
            {
                filePath [0] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"\output-dico\charset";
                filePath [1] = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"\output-dico\wordlist";
